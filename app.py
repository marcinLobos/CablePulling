...
# --- KONFIGURACJA ---
st.set_page_config(page_title="Pull-Planner v3.1", layout="wide")
...
def zastosuj_motyw(tryb):
    if tryb == "Dark":
        st.markdown("""<style>
            ...
            .stTable { background-color: #1d2129; }
            </style>""", unsafe_allow_html=True)
    else:
        st.markdown("""<style>.stApp { background-color: #ffffff; color: #000000; }</style>""", unsafe_allow_html=True)

# --- PEŁNY SŁOWNIK (Rozbudowany) ---
TLUMACZENIA = {
    "PL": {
        "tytul": "⚡ Profesjonalny Planer Naciągu Kabli (v3.1)",
    ...
    "EN": {
        "tytul": "⚡ Professional Cable Pull-Planner (v3.1)",
        "motyw": "Theme:",
        "naciag": "Tension",
...
with st.sidebar:
    ...
    t_start = st.number_input(f"{txt['naciag_pocz']} ({j_sila})", value=0.0)
    limit_uzyt = st.number_input(f"Dopuszczalny limit ({j_sila})", value=10.0 if j_sila=="kN" else 5000.0)
    ...
    if typ_oslony == txt["o_rura"]:
        ...
        D_wew = 999.0 # Wyłączenie jammingu
    ...
    if st.button(f"➕ {txt['dodaj']}"): st.session_state.kable.append({"d": c_d, "w": c_w})
    if st.session_state.kable:
        st.table(pd.DataFrame(st.session_state.kable))
        if st.button("🗑️ Wyczyść kable"): st.session_state.kable = []; st.rerun()
...
if st.session_state.kable:
    ...
    if typ_oslony == txt["o_rura"]:
        if 2.8 <= jam_ratio <= 3.2: 
            st.error(txt["jam_error"] + " (Jam Ratio w zakresie 2.8-3.2)")
        else:
            st.success("✅ Brak ryzyka zaklinowania.")

# --- TRASA ---
if 'trasa' not in st.session_state: st.session_state.trasa = []
st.subheader(txt["trasa"])
col1, col2, col3 = st.columns([2, 2, 3])
with col1: typ_odc = st.selectbox("Element", [txt["prosta"], txt["luk"]])
with col2: val_odc = st.number_input("Dł. / Kąt", value=10.0)
with col3:
    if typ_odc == txt["prosta"]:
        t_nach = st.radio("Nachylenie:", ["°", "%"], horizontal=True)
        nach = st.number_input("Kąt/Procent", value=0.0)
        r_luk = 0.0
    else:
...
if st.session_state.trasa:
    ...
    sc1.metric("Całkowita dł. trasy", f"{round(total_L, 1)} {u_len}")
    sc2.metric("Łączna waga kabli", f"{round(waga_total, 2)} jedn.")
    sc3.metric("Współczynnik WC", round(wc, 3))

    f_ekran = naciag_N * m_ekr
    if naciag_N > limit_N:
        st.error(f"{txt['alarm']} ({round(f_ekran, 2)} {j_sila})")
    else:
        st.success(f"{txt['bezpiecznie']} ({round(f_ekran, 2)} {j_sila})")

    if st.button("🗑️ Wyczyść trasę"): st.session_state.trasa = []; st.rerun()